...
class PineconeStore(BaseStore):
    ...
    def add_documents(
        ...
        logger.info(f"Added {len(documents)} documents to Pinecone index {self.collection_name}")
            ...
            if ids:
                ...
                if query_results and "matches" in query_results:
                    ...
                    if ids_to_delete:
                        ...
                        logger.debug(f"Deleted {len(ids_to_delete)} documents from Pinecone based on filter")
            else:
