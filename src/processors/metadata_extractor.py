...
def extract_metadata(
    content: str, language: str, file_path: str, chunk_index: int
) -> Dict[str, Any]:
    """
...
def determine_chunk_type(content: str, language: str, file_path: str) -> str:
    """
...
def extract_csharp_metadata(content: str) -> Dict[str, Any]:
    ...
    method_match = re.search(
        r"(?:public|private|protected|internal)?\s*(?:static|virtual|abstract|override)?\s*\w+\s+(\w+)\s*\(",
        content,
    )
    if method_match:
...
def extract_js_metadata(content: str) -> Dict[str, Any]:
    ...
    if component_match:
        ...
        if (
            component_name and component_name[0].isupper()
