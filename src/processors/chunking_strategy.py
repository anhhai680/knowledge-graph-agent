...
class ChunkingStrategy(ABC):
    ...
    def _preserve_metadata(
        self, original_document: Document, chunks: List[str]
    ) -> List[Document]:
        """
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_document = Document(page_content=chunk, metadata=metadata)
    ...
    def split_document(self, document: Document) -> List[Document]:
        ...
        for class_name, class_content in class_chunks.items():
            ...
            if not method_chunks:
                ...
                chunk_document = Document(page_content=class_content, metadata=metadata)
                ...
                for method_name, method_content in method_chunks.items():
                    ...
                    chunk_document = Document(
                        page_content=method_content, metadata=metadata
                    )
...
class ReactChunkingStrategy(ChunkingStrategy):
    ...
    def split_document(self, document: Document) -> List[Document]:
        ...
        for component_name, component_content in component_chunks.items():
            ...
            chunk_document = Document(page_content=component_content, metadata=metadata)
        ...
        for function_name, function_content in function_chunks.items():
            ...
            if any(
                function_content in component for component in component_chunks.values()
            ):
                continue
            ...
            chunk_document = Document(page_content=function_content, metadata=metadata)
