...
class WorkflowStateMetadata(BaseModel):
    ...
    serialization_format: StateSerializationFormat = Field(
        default=StateSerializationFormat.JSON,
        description="Serialization format used"
    )
...
class FileStateManager(StateManager):
    ...
    def _get_state_file_path(self, workflow_id: str) -> Path:
        ...
        Returns:
            Path to state file
        """
        extension = "json" if self.serialization_format == StateSerializationFormat.JSON else "pkl"
    ...
    def load_state(self, workflow_id: str) -> Optional[StateType]:
        ...
        try:
            ...
            self.logger.debug(f"Loaded state for workflow {workflow_id} from {state_file}")
            return state
    ...
    def get_state_metadata(self, workflow_id: str) -> Optional[WorkflowStateMetadata]:
        ...
        try:
            ...
            self.logger.error(f"Failed to load metadata for workflow {workflow_id}: {e}")
            return None
    ...
    def create(
        ...
        **kwargs
        ...
        if backend == StateBackend.MEMORY:
            ...
            logger.warning("Database state manager not implemented, falling back to file")
            return FileStateManager(**kwargs)
        else:
