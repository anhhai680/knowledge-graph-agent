...
class EmbeddingFactory:
    ...
    def batch_embed_documents(
        ...
        max_tokens_per_batch = (
            max_tokens_per_batch or settings.embedding.max_tokens_per_batch
        )
        ...
        for doc, token_count in zip(documents, doc_token_counts):
            ...
            if (
                current_batch
                and current_batch_tokens + token_count > max_tokens_per_batch
            ):
        ...
        for i, batch in enumerate(batch_iterator):
            try:
                ...
                logger.debug(
                    f"Embedded batch {i+1}/{total_batches} ({len(batch)} documents)"
                )
                ...
                if half_size > 0:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Embedded first half of batch {i+1} ({len(first_half)} documents)"
                        )
                    except Exception as e2:
                        logger.error(
                            f"Error embedding first half of batch {i+1}: {str(e2)}"
                        )
                        ...
                        for doc in first_half:
                            try:
                                ...
                                logger.error(
                                    f"Error embedding individual document: {str(e3)}"
                                )
                                # Add a zero vector as placeholder
                                all_embeddings.append(
                                    [0.0] * LLM_CONSTANTS.EMBEDDING_DIMENSION.value
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Embedded second half of batch {i+1} ({len(second_half)} documents)"
                        )
                    except Exception as e2:
                        logger.error(
                            f"Error embedding second half of batch {i+1}: {str(e2)}"
                        )
                        # If still failing, embed one by one
                    ...
                    logger.error(
                        f"Cannot split batch further, adding zero vectors for {len(batch)} documents"
                    )
                    all_embeddings.extend(
                        [[0.0] * LLM_CONSTANTS.EMBEDDING_DIMENSION.value] * len(batch)
                    )  # Assuming OpenAI's embedding size
