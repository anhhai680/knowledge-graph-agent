                ...
                page_content="""
{
    ...
    "license": "MIT"
        ...
        with (
            ...
            for file_state in final_state["file_processing_states"]:
                ...
                self.assertIn(
                    file_state["file_path"], ["hello.py", "README.md", "package.json"]
                )
            ...
            logging.info(
                f"Embeddings generated: {final_state['successful_embeddings']}"
            )
            logging.info(
                f"Processing rate: {final_state.get('documents_per_second', 0):.2f} docs/sec"
            )
            logging.info(
                f"Embedding rate: {final_state.get('embeddings_per_second', 0):.2f} embeddings/sec"
            )

    def test_workflow_error_recovery(self):
        """Test workflow error recovery mechanisms."""
        initial_state = create_indexing_state(
            workflow_id="error-recovery-test", repositories=["test-integration-repo"]
        )
        ...
        with patch("src.workflows.indexing_workflow.GitHubLoader") as mock_loader_class:
            ...
            with (
                ...
                mock_processor.process_document.return_value = [
                    Document(
                        page_content="chunk",
                        metadata={"file_path": "test.py", "chunk_type": "content"},
                    )
                ]
                mock_doc_processor_class.return_value = mock_processor
    ...
    def test_workflow_state_persistence_simulation(self):
        ...
        initial_state = create_indexing_state(
            workflow_id="persistence-test", repositories=["test-integration-repo"]
        )
        ...
        with (
            patch("src.workflows.indexing_workflow.GitHubLoader") as mock_loader_class,
